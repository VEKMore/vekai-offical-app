...
import { isMemberMode } from '../lib/auth';
            ...
            <MasonrySimple
              ...
              renderItem={(scene) => (
                <article key={scene.id} className="overflow-hidden rounded-4xl border border-white/10 bg-cyberPanelDeep shadow-glow-sm transition duration-300 hover:-translate-y-1">
                  ...
                  <div className="p-5">
                  <PrimaryButton
                    className="w-full text-2xs px-4 py-3 btn-small"
                    variant="primary"
                    icon="cast"
      ...
      {previewScene && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-xl rounded-[2rem] border border-white/10 bg-cyberPanel/95 p-6 shadow-glow">
            ...
            <p className="mt-4 text-sm leading-7 text-cyberGray">Dive into this featured scene, explore its role setup, and move quickly from inspiration to production.</p>
            <div className="mt-5 flex flex-wrap gap-3">
              <PrimaryButton href="/explore" variant="primary" icon="search">
                Explore more
              </PrimaryButton>
              <PrimaryButton onClick={() => setPreviewScene(null)} variant="secondary" icon="cast">
                Keep browsing
              </PrimaryButton>
            </div>
          </div>
