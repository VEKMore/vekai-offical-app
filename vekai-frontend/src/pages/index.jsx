      ...
      <main className="flex-1 p-6 lg:p-10 flex flex-col gap-8 overflow-y-auto max-w-7xl mx-auto w-full">
        ...
        <section className="w-full bg-gradient-to-r from-purple-900/20 via-[#0A0A0C] to-[#0A0A0C] border border-white/10 p-8 rounded-xl flex flex-col lg:flex-row justify-between items-center gap-6 shadow-2xl relative overflow-hidden">
          ...
          <div className="w-[280px] aspect-[16/10] bg-neutral-900 border border-white/10 flex items-center justify-center text-center p-4 rounded-md">
            <span className="text-white/20 text-[9px] font-mono tracking-widest uppercase">[ Preview ]</span>
          </div>
        </section>

        {/* EXCLUSIVE MERCHANDISE SECTIONS */}
        <section className="w-full flex flex-col gap-4">
          <div className="w-full flex justify-between items-end border-b border-white/5 pb-2">
            ...
            <div className="bg-[#FF007F]/10 border border-[#FF007F]/30 text-[#FF007F] font-mono text-xs px-3 py-1.5 tracking-widest rounded-sm font-bold">
        ...
        {/* PRODUCTION CATALOGUE GRIDS */}
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {filteredProducts.map((product) => (
            <div key={product.id} className="bg-[#0A0A0C] border border-white/5 p-4 rounded-lg flex flex-col justify-between gap-4 group transition-all hover:border-white/15">
              <div className="w-full aspect-[4/3] bg-[#111217] border border-white/5 flex flex-col justify-center items-center p-3 relative rounded-sm">
                <span className="text-white/5 font-mono text-[9px] tracking-widest uppercase">[ {product.type} ]</span>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-gray-500 text-[8px] font-mono tracking-widest uppercase block">{product.type}</span>
                <h4 className="text-white font-black text-xs leading-tight tracking-wide group-hover:text-[#00FFCC] transition-colors line-clamp-2 uppercase min-h-[32px]">{product.name}</h4>
{product.desc}{product.price}<button onClick={() => setCart(prev => [...prev, product.id])} className="w-full bg-transparent border border-white/10 text-gray-400 group-hover:text-black group-hover:bg-[#00FFCC] group-hover:border-[#00FFCC] font-black py-2.5 text-[10px] tracking-widest uppercase transition-all rounded-sm">Details))}{/* SCENE COLLECTIONS MATRIX BUNDLES */}{/* FLOAT TRYS HUD DISPLAY */}{cart.length > 0 && (🛍️ CART COUNT{cart.length}<button onClick={() => setCart([])} className="text-[9px] border-l border-black/20 pl-2 opacity-60 hover:opacity-100 uppercase">Clear)});}