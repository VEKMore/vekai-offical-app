          ...
          {MERCH_ITEMS.map((item) => (
            <div key={item.id} className="space-y-4 overflow-hidden rounded-3xl border border-white/10 bg-cyberPanelDeep p-6 shadow-glow-sm">
              ...
              <div className="flex flex-wrap gap-3">
                <button className="rounded-2xl bg-cyberPurple px-4 py-3 text-sm font-black uppercase tracking-wide text-white transition hover:bg-cyberPurpleSoft">Add to cart</button>
                <button className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-black uppercase tracking-wide text-white/80 transition hover:bg-white/10">Details</button>
              </div>
            </div>
          ))}
        </section>
